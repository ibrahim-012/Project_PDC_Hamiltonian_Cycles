...
# Parse the data into a list of tuples (input_size, num_processes, time)
data = []
for line in lines:
    input_size, num_processes, time = line.split()
    data.append((int(input_size), int(num_processes), float(time)))

# Sort the data based on input_size, then num_processes, then time
data = sorted(data, key=lambda x: (x[0], x[1], x[2]))

# Remove duplicates by keeping only the entry with the smallest time
filtered_data = {}
for input_size, num_processes, time in data:
    key = (input_size, num_processes)
